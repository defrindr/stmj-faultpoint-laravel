...
<div class="col-10">
    ...
    <form method="post" action="/point/{{ $point->id }}">
    @method('patch')
    @csrf    
  ...
  <div class="form-group">
    ...
    placeholder="Masukkan peraturan" name="peraturan" value="{{ $point->peraturan }}">
    @error('peraturan')
  ...
  <div class="form-group">
    ...
    placeholder="Masukkan interval_waktu" name="interval_waktu" value="{{ $point->interval_waktu }}">
  ...
  <div class="form-group">
    ...
    placeholder="Masukkan point" name="point" value="{{ $point->point }}">
    @error('point')
    <div class="invalid-feedback">{{ $message }}</div>
    @enderror
  </div>
  <div class="form-group">
    ...
    placeholder="Masukkan sanksi" name="sanksi" value="{{ $point->sanksi }}">
    @error('sanksi')
    <div class="invalid-feedback">{{ $message }}</div>
    @enderror
  </div>
  ...
  <div class="form-group">
    ...
    name="kategori_point_id" >

    @foreach ($KategoriPoint as $rows)
    <option value="{{ $rows->id }} " 
        @if($rows->id === $point->kategori_point_id) selected 
        @endif> {{ $rows->nama }}
    </option>
    @endforeach
